Shader "Custom/MyShader"
{
	Properties
	{
		_MainTex("Texture", 2D) = "white" {}
		_Shininess("Shininess", Range(1, 20)) = 10
	}
    SubShader
    {
		Pass
		{
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag
			#pragma shader_feature TYPE_NORMAL
			#pragma shader_feature TYPE_BLINN
			#pragma shader_feature TYPE_PHONG
			#pragma shader_feature USE_DIFFUSE
			#pragma shader_feature USE_AMBIENT
			#pragma shader_feature USE_SPECULAR

			#include "UnityCG.cginc"
			#include "UnityStandardBRDF.cginc"

			struct VertexData
			{
				float4 vertex : POSITION;
				float3 normal : NORMAL;
				float2 uv : TEXCOORD0;
			};

			struct FragmentData
			{
				float4 vertex : SV_POSITION;
				float2 uv : TEXCOORD0;
				float3 normal : TEXCOORD1;
				float3 worldPos : TEXCOORD2;
			};

			sampler2D _MainTex;
			float _Shininess;
			float4 _MainTex_ST;

			FragmentData vert(VertexData v)
			{
				FragmentData i;
				i.vertex = UnityObjectToClipPos(v.vertex);
				i.normal = UnityObjectToWorldNormal(v.normal);
				i.uv = TRANSFORM_TEX(v.uv, _MainTex);
				i.worldPos = mul(unity_ObjectToWorld, v.vertex);
				return i;
			}

			fixed4 frag(FragmentData i) : SV_Target
			{
#if TYPE_NORMAL
				return float4(i.normal, 1);
#endif
				float3 lightDir = _WorldSpaceLightPos0.xyz;//外界光照方向（原点指向光源）
				float3 lightColor = _LightColor0.rgb;//外界光照颜色
				float3 viewDir = normalize(_WorldSpaceCameraPos - i.worldPos);//视线方向（表面指向相机）

				float4 col = tex2D(_MainTex, i.uv);
				//diffuse light
				float3 diffuse = col.rgb;
#if USE_DIFFUSE
				diffuse = col.rgb * DotClamped(lightDir, i.normal);
#endif
				//ambient light
				float3 ambient = float3(0, 0, 0);
#if USE_AMBIENT
				ambient = col.rgb * UNITY_LIGHTMODEL_AMBIENT.xyz;
#endif
				//specular light
				float3 specular = float3(0, 0, 0);
#if USE_SPECULAR
	#if TYPE_BLINN
				float3 halfVector = normalize(lightDir + viewDir);//仅在平行光情况下L等于外界光照方向
				specular = pow(max(0, dot(halfVector, i.normal)), _Shininess) * lightColor;
	#endif
	#if TYPE_PHONG
				float3 reflection = normalize(dot(lightDir, i.normal) * 2.0 * i.normal - lightDir);//R + L = R·L * 2N，反射方向
				specular = pow(max(0, dot(viewDir, reflection)), _Shininess) * lightColor;
	#endif
#endif
				return float4(diffuse + ambient + specular, 1);
			}
			ENDCG
		}
    }
    FallBack "Diffuse"
	CustomEditor "CustomShaderGUI"
}
