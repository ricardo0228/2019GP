...
public class drawOval : MonoBehaviour
{
    ...
    private Vector3 r;
    private Vector3 v;
    private enum TrajectoryType { Oval, Circle, Parabola, Hyperbola };
    ...
    void FixedUpdate()
    {
        ...
        if (Vector3.Cross(r, v).z > 0)
            SetPosition(t0 + t);
        else if (Vector3.Cross(r, v).z < 0)
            SetPosition(t0 - t);
        else if (Vector3.Cross(r, v).y < 0)
            SetPosition(t0 + t);
        else
            SetPosition(t0 - t);
    ...
    private void AddPoints()
    {
        ...
        for (int i = 0; i <= num; i++)
        {
            ...
            if (type == TrajectoryType.Oval || type == TrajectoryType.Circle)
            {
    ...
    {
        if (type == TrajectoryType.Hyperbola || type == TrajectoryType.Parabola ) return;
        ...
        if(type == TrajectoryType.Oval)
        {
            ...
            x0 = w * w / f - w / f * absR;
            y0 = Mathf.Sqrt(Mathf.Abs(absR * absR - (x0 - f) * (x0 - f)));
            if (beta < 0)
                y0 = -y0;
        }
        if (type == TrajectoryType.Circle)
        {
            beta = time / T * 2 * Mathf.PI;
            print(time);
            print(T);
            print(beta);
            x0 = w * Mathf.Cos(beta);
